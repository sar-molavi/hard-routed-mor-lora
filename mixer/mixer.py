...
from .moe import RSL, LoRAExperts
    ...
    def forward(
        ...
        *,
        ...
        def full_forward(input_x: torch.Tensor):
            scores, aux_loss, topk_indices, topk_weights = self.router(
        ...
        if (
            ...
            down, aux_loss, scores = _checkpoint(full_forward, x)
        else:
            down, aux_loss, scores = full_forward(x)
...
class LoRAMixerFFN(nn.Module):
    ...
    def __init__(
        ...
        self._choices: torch.Tensor | None = None
        self.alpha = alpha
    ...
    def set_router_temperature(self, temperature: float) -> None:
        ...
        if self.router_shared_across_layers:
            if self.shared_router is not None:
                self.shared_router.set_gumbel_temperature(temperature)
            return

        for router in self.router_layers.values():
            router.set_gumbel_temperature(temperature)
            ...
            def make_mlp_forward(router_block: LoRAFFNBlock):
                def new_mlp_forward(x: torch.Tensor) -> torch.Tensor:
                    out, aux_loss = router_block.forward(
                        x, attention_mask=self._attention_mask, choices=self._choices
                    )
