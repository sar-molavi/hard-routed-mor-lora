    ...
    def forward(
        ...
        Returns:
            ...
            - topk_weights (torch.FloatTensor): Normalized weights for the top-k experts,
              shape `(batch_size, seq_len, k)`.
            - router_weights (torch.FloatTensor): Dense routing weights after top-k
              selection/normalization, shape `(batch_size, seq_len, num_experts)`.
        ...
        def full_forward(input_hidden_states: torch.Tensor):
            ...
            return scores, aux_loss, topk_indices, topk_weights, router_weights
        ...
        if (
            ...
            def checkpoint_forward(input_hidden_states: torch.Tensor):
                scores, aux_loss, topk_indices, topk_weights, router_weights = full_forward(
                    input_hidden_states
                )
                if aux_loss is None:
            ...
            scores, aux_loss, topk_indices, topk_weights, router_weights = _checkpoint(
